...
def get_RHS_matrices(energy_nodes, sigma_array, sig3_array, dxs_array, sec_array, regen_array):
...
def get_eigs(flavor, gamma, h5_filename, ato=None, pure_spectrum=False):
    """ Returns the eigenvalues for a given flavor, spectral index, and energy range.

    Args:.
        flavor: specifies the neutrino flavor of interest. Negative numbers for
    ...
    if flavor == -1:
        ...
        raise ValueError(
            "OH NO! You need to specify a flavor that is not tau (= 3 or -3). About to crash because of this.")
...
def get_glashow_total(energy_nodes, ato=None):
    ...
    sig = 1.e0 / 3.e0 * GF ** 2 * selectron / np.pi * (1. - (mmu ** 2 - me ** 2) / selectron) ** 2 / (
                (1. - selectron / MW ** 2) ** 2 + GW ** 2 / MW ** 2) * .676 / .1057 * hbarc ** 2
    if ato is None:
        return sig
